...
public class HopitalApplication implements CommandLineRunner {
    ...
    public void run(String... args) throws Exception {
        ...
        patient.setName("Houda");
        patient.setDateNaissance(new Date());
        patient.setMalade(false);
        patient.setScore(120);
        patientRepository.save(patient);

        Patient patient2=new Patient(null,"Ahlam",new Date(),false,134);
    ...
    CommandLineRunner commandLineRunner(JdbcUserDetailsManager jdbcUserDetailsManager){
        ...
        return args -> {
            ...
            if (u3 == null)
                jdbcUserDetailsManager.createUser(
                    User.withUsername("admin2").password(passwordEncoder().encode("1234")).roles("ADMiN").build()
            );
        };
    }

    //@Bean
    CommandLineRunner commandLineRunnerUsers(AccountService accountService){
        return args -> {
